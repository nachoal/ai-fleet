...
class WorktreeManager:
    ...
    def create_worktree(self, branch: str, path: Optional[Path] = None) -> Optional[Path]:
        ...
        success, stdout, _ = self._run_git(["show-ref", "--verify", f"refs/heads/{branch}"])
        branch_exists = success
        
        # Create worktree
        if branch_exists:
            # Use existing branch
            success, stdout, stderr = self._run_git(["worktree", "add", str(path), branch])
        else:
            # Create new branch
            success, stdout, stderr = self._run_git(["worktree", "add", str(path), "-b", branch])
    ...
    def setup_worktree(self, branch: str, credential_files: List[str], 
                      setup_commands: List[str], quick_setup: bool = False) -> Optional[Path]:
